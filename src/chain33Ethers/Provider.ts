...
export class BtySigner extends Signer implements TypedDataSigner {
    declare readonly provider: BtyProvider;
    _index: number;
    _address: string;
    ...
    connect(provider: Provider): BtySigner {
        return logger.throwError("cannot alter JSON-RPC Signer connection", Logger.errors.UNSUPPORTED_OPERATION, {
            operation: "connect"
        });
    }

    connectUnchecked(): BtySigner {
...
class UncheckedJsonRpcSigner extends BtySigner {
...
export class BtyProvider extends BaseProvider {
    ...
    async createRawTransaction(rawTransaction:RawTransaction):Promise<{rawTx:string,sha256Hash:string}>{
        return  await this.perform('createRawTransaction',rawTransaction)
    }
    ...
    async perform(method: string, params: any): Promise<any> {
        ...
        if(method === 'getStorageAt' || method === 'sendRawTransaction'){
            logger.throwError(method + " not supported by Bty-Chain", Logger.errors.UNSUPPORTED_OPERATION, { operation: method });
        }
    ...
    static hexlifyTransaction(transaction: TransactionRequest, allowExtra?: { [key: string]: boolean }): { [key: string]: string | AccessList } {
        ...
        // console.log(allowed);
        if (allowExtra) {
            console.log('allowExtra',allowExtra);
